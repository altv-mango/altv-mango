...
export function OnInternal(name: string) {
    return <MethodDecorator>((target: Object, method: string) => {
        ...
        if (events.some((event) => event.method === method && event.name === name && event.type === 'onInternal')) {
        ...
        Reflect.defineMetadata<Omit<EventMetadata, 'params' | keyof PipelineMetadata>[]>(
            CoreMetadataKey.ControllerEvents,
            [...events, { method, name, type: 'onInternal' }],
            target,
        );
    });
}
