...
export function OnceWebView(id: string | number, name?: string) {
    return <MethodDecorator>((target: Object, method: string) => {
        ...
        if (events.some((event) => event.method === method && event.name === name && event.type === 'onceWebView')) {
        ...
        Reflect.defineMetadata<Omit<EventMetadata, 'params' | keyof PipelineMetadata>[]>(
            CoreMetadataKey.ControllerEvents,
            [...events, { method, name, type: 'onceWebView', webViewId: id }],
            target,
        );
    });
}
