...
export class ModuleTreeScanner {
    ...
    public async scan(classRef: Newable | DynamicModule | Promise<DynamicModule>) {
        ...
        this.loggerService.log(
            [chalk.magentaBright('Scan'), chalk.blueBright(module.metadata.classRef.name)],
            `→ [ providers: ${module.metadata.internalProviders.size + module.metadata.externalProviders.size} | controllers: ${
                module.metadata.controllers.length
            } | imports: ${module.metadata.imports.length} | exports: ${module.metadata.externalProviders.size} ]` +
    ...
    private async scanControllers(module: Module) {
        ...
        for (const classRef of module.metadata.controllers) {
            ...
            this.loggerService.log(
                [chalk.magentaBright('Scan'), chalk.cyanBright(classRef.name)],
                `→ [ events: ${controllerMetadata.events.length} | rpcs: ${controllerMetadata.rpcs.length} ]` +
                    chalk.gray(` (${Date.now() - startTime}ms)`),
            );
        }
    }
